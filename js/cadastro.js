...
function insereNaTabela(id, nome, fone, sexo, cidade) {
    let tabela = document.getElementById('lista-de-contatos').getElementsByTagName('tbody')[0];
...
function buscaContatoPeloId(id) {
    let body = document.getElementById('lista-de-contatos').getElementsByTagName('tbody')[0];
    let quantidadeDeLinhas = body.rows.length;
    for (let i = 0; i < quantidadeDeLinhas; i++) {
        if (body.rows[i].cells[0].innerHTML == id) {
            ...
            let sexo = body.rows[i].cells[3].innerHTML.toLocaleLowerCase();
            if (sexo == 'masculino') {
                document.getElementById('masculino').checked = true;
